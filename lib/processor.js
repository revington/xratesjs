...
function getContextOrValue(ctxt, register, value) {
    if (_.isNumber(value)) {
        return value;
    }
    if (!_.isUndefined(ctxt.registers[value])) {
        return ctxt.registers[value];
    }
    return value;
}

function add(ctxt, register, mode, value, verb) {
    ctxt.registers[register] += getContextOrValue(ctxt, register, value);
}
...
function createRegister(ctxt, register, mode, value, verb) {
    var v = getContextOrValue(ctxt, register, value);
    ctxt.registers[register] = _.isNumber(v) ? 0 : '';
}
    ...
    if (step.children) {
        ...
        while (i--) {
            if (i === 0) {
                ...
                // TODO: is branching really necessary?
                //            } else {
...
function reckon(input) {
    ...
    _.forEach(input.applied, function (applied) {
        _.forEach(applied.units, function (u) {
             ...
             */
            isOnce = (applied.frequencyType === 'Once' && applied.unitType === 'Once');
            target = isOnce ? u.unit : u;
            ...
            /**
             * Update main registers if we are not on a *once* update
             */
            if (!isOnce) {
